...
- Simplified Kotlin JVM target configuration by using `JvmTarget.fromTarget(javaVersion)` from a Gradle property instead
  of a manual `when` statement.
...
- Migrate to Gradle IntelliJ Plugin 2.x
    - Updated build scripts to use `intellijPlatform {}` extension
